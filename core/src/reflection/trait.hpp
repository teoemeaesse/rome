...
public:
  ...
  inline b8 operator==(const Trait &other) const noexcept {
   ...
   */
  template <typename T> static inline UUID getUUID() {
    RM_ASSERT_MSG((std::is_base_of_v<Trait, T>), "T is not a trait");
   ...
   */
  template <typename T> static inline b8 is(const Trait &trait) {
    RM_ASSERT_MSG((std::is_base_of_v<Trait, T>), "T is not a trait");
...
private:
  ...
  const char
      *name; ///< The name of the trait. Should be unique for each trait type.
};
} // namespace rome::core
